...
- **Urgency to save:** By having feedback on a daily basis, a sense of urgency can be felt. This urgency prevents household finance related decisions to be postponed and ignored until there is a unpleasant bank report. This makes awareness and response regarding the household finances a habit. 
...
The application will refrain from trying to apply [traditional kakeibo techniques](https://www.google.com.br/books/edition/Kakeibo/BmJ5tAEACAAJ?hl=en), such as defining 固定費 (fixed-value recurring expenses), 変動費 (varied-value recurring expenses) or やりくり費 (expenses that are not necessarily recurring), instead putting all of that responsibility on the user through the tags system. Each transaction can receive an arbitrary amount of tags (which are registered in a specific table), so consistency is expected to make future queries be useful.
...
- **--currency, -c** -> sets the currency of the transaction using the 3-characters currency codes defined on [ISO 4217](https://www.iso.org/iso-4217-currency-codes.html), with the currency defined in the configuration file being the default
